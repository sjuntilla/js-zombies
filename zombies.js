 ...
 */
class Player {
  constructor(name, health, strength, speed, pack) {
    this._pack = [];
  ...
  discardItem(item) {
    ...
    if (pack.indexOf(item) >= 0) {
      ...
      console.log(this.name + ' throws out a ' + item + '.');
      return true;
    }
    ...
    if (itemToEquip instanceof Weapon) {
      if (pack.indexOf(itemToEquip) >= 0) {
        if (this.equipped) {
          ...
          pack.splice(pack.indexOf(itemToEquip), 1, swap);
        } else {
          this.equipped = pack[pack.indexOf(itemToEquip)];
      ...
      console.log('How would you even fight with this?')
    }
  ...
  eat(itemToEat) {
    ...
    if (itemToEat instanceof Food) {
      if (pack.indexOf(itemToEat) >= 0) {
        ...
        console.log('What are you trying to eat?')
      }
    } else {
